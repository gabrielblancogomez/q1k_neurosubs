...
setwd("C:/Users/gabot/OneDrive - McGill University/Desktop/Github_repos/q1k_neurosubs/code/linear_models/Alpha_Power")
...
# Ensure outcome variables are numeric
 outcome_vars <- c("pwr_frontal_rel")
...
### SECTION 2 Outlier detection
 ...
 # Remove outliers for pwr_frontal_rel
 data <- data[!data$pwr_frontal_rel_outlier, ]
...
# Save plot
ggsave("./output/figures/age_distribution_by_group.png", width=8, height=6, dpi=300)
...
p_var
ggsave(filename = paste0("output/diagnostics/variance_", outcome, ".png"),
...
# Scatteplot with LOESS Smooth
p_age <- ggplot(data_complete, aes(x = eeg_age, y = .data[[outcome]], color = affected_group)) +
  geom_point(alpha = 0.4, size = 2) +
  geom_smooth(method = "loess", se = TRUE, size = 1.2, span = 0.75) +
  scale_color_manual(values = c("Non-Affected" = "#2ecc71",
                                "Affected (non-ASD)" = "#3498db",
                                "ASD" = "#e74c3c")) +
  labs(title = paste(outcome, "across Age by Group"),
       subtitle = "LOESS smooths show observed trends",
       x = "Age (years)", y = outcome, color = "Group") +
  theme_minimal(base_size = 12) +
  theme(legend.position = "bottom")
p_age

ggsave(filename = paste0("output/figures/age_relationship_", outcome, ".png"),
       plot = p_age, width = 10, height = 7, dpi = 300)
...
# Save plot
ggsave(filename = paste0("./output/figures/age_relationship_by_sex", outcome, ".png"),
       plot = p_age_sex, width = 10, height = 7, dpi = 300)

# Raw by sex 
p_raw_sex <- ggplot(data_age_restricted, aes(x = eeg_age, y = data_age_restricted[[outcome]], color = sex)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "gam", formula = y ~ s(x, k = 5), se = TRUE) +
  facet_wrap(~affected_group) +
  labs(title = "Trajectories by Sex and Group") +
  theme_minimal()
...
prelim_model <- lm(l_index ~ eeg_age + affected_group + sex + site, data = data_complete)
...
print("**BEST MODEL SEEMS TO BE QUADRATIC***")
...
formula_m1 <- as.formula(paste(outcome, "~ 1"))
m1 <- lmer(formula_m0, data = data, REML = TRUE)
...
formula_m2 <- as.formula(paste(outcome,
                               "~  poly(eeg_age,2) + sex + site"))

m2 <- lm(formula_m2, data = data_model)
...
formula_m3 <- as.formula(paste(outcome,
                               "~ affected_group * poly(eeg_age,2) + sex + site"))

m3 <- lm(formula_m3, data = data_model)
...
lr_test <- anova(m2, m3, test = "Chisq")
print("\nLikelihood Ratio Test (M2 vs M3):")
print(lr_test)



# MODEL 4 : INTERACTION SEX AND AGE
formula_m4 <- as.formula(paste(outcome,
                               "~ affected_group *sex * poly(eeg_age,2) + site"))

m4 <- lm(formula_m4, data = data_model)
...
cat("Model R²:", round(r2_m4$R2, 3), "\n")
cat("adj. R²:", round(r2_m4$R2, 3), "\n")


# MODEL COMPARISON TABLE
comparison_table <- data.frame(
  Model = c("M1: Null", "M2: Main Effects", "M3: Interaction (LMER)",
            "M4: Interaction (LMER)"),
  AIC = c(aic_m1, aic_m2, aic_m3, aic_m4),
  BIC = c(bic_m1, bic_m2, bic_m3, bic_m4),
  R2_marginal = c(
    0,  # M0 has no fixed effects
    performance::r2(m2)$R2_marginal,
    performance::r2(m3)$R2_marginal,
    performance::r2(m4)$R2_marginal
  ),
  R2_conditional = c(
    performance::r2(m1)$R2_conditional,
    performance::r2(m2)$R2_conditional,
    performance::r2(m3)$R2_conditional,
    performance::r2(m4)$R2_conditional
  )
)
...
print(paste("Marginal R² (fixed effects):", round(r2_vals$R2_marginal, 4)))
print(paste("Conditional R² (fixed + random):", round(r2_vals$R2_conditional, 4)))
...
# F. Residuals vs predictors
p_res1 <- ggplot(resid_df, aes(x = eeg_age, y = residuals, color = affected_group)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "loess", se = FALSE) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Residuals vs Age", x = "Age (years)", y = "Residuals") +
  theme_minimal()

p_res2 <- ggplot(resid_df, aes(x = fitted, y = residuals)) +
  geom_point(alpha = 0.5) +
...
(p_res1 | p_res2)
ggsave("output/diagnostics/residual_plots.png", width = 12, height = 5, dpi = 300)
...
# SECTION 13: POWER ANALYSIS
pwr_result <- pwr.f2.test(
  u = 7,  # numerator df (3 groups, age, sex, site, interaction)
  v = 231,  # denominator df
  f2 = NULL,
  sig.level = 0.05,
  power = 0.80
)
...
group_means <- data_complete %>%
  group_by(affected_group) %>%
  summarise(M = mean(l_index), SD = sd(l_index))
...
# Generate predictions across age range for each group
pred_data <- ggpredict(m3, 
                       terms = c("eeg_age [all]", "affected_group"),
                       type = "fixed")  # Fixed effects only (population average)

# Plot predicted trajectories
p_pred <- plot(pred_data) +
  scale_color_manual(
  ...
  labs(
    title = "Predicted Developmental Trajectories (Model M2)",
    subtitle = "Lines: model predictions | Ribbons: 95% CI",
    x = "Age (years)",
    y = "Predicted Log Alpha Power",
    color = "Group"
  ) +
  theme_minimal(base_size = 14)

print(p_pred)
ggsave("output/figures/predicted_trajectories_m2.png", 
       plot = p_pred, width = 10, height = 7, dpi = 300)



# REPORT MAIN EFFECTS

emm_group <- emmeans(m3, ~ affected_group)
...
# Subset data
data_age_restricted <- data %>% 
  filter(eeg_age >= common_min & eeg_age <= common_max)
...
best_age_restricted <- lm(formula_m3,
  data = data_age_restricted,
)
