...
setwd("C:/Users/gabot/OneDrive - McGill University/Desktop/Github_repos/q1k_neurosubs/code/linear_models/Connectivity")
...
data$left <- log(data$left)

data_complete <- data[!is.na(data[[outcome]]), ]

# WINSORIZE JUST IN CASE
library(DescTools)
data$left <- Winsorize(data$left, 
                       quant = c(0.01, 0.99))
...
prelim_model <- lm(left ~ eeg_age + affected_group + sex + site, data = data_complete)
...
formula_m2 <- as.formula(paste(outcome,
                               "~ affected_group + s(eeg_age, k=5) + sex +  s(fam_id, bs='re')"))
...
formula_m3 <- as.formula(paste(outcome,
                               "~ affected_group + s(eeg_age, by=affected_group, k=5) + sex +  s(fam_id, bs='re')"))
...
group_means <- data_complete %>%
  group_by(affected_group) %>%
  summarise(M = mean(left), SD = sd(left))
...
best_age_restricted <- gam(formula_m4,
...
p <- ggplot(data_age_restricted, aes(x = eeg_age, y = left, color = affected_group)) +
  geom_point(alpha = 0.3, size = 2) +
  geom_smooth(method = "gam", formula = y ~ s(x, k=5), 
              aes(fill = affected_group), alpha = 0.2) +
  labs(title = "Connectivitiy Trajectories by Group",
       x = "Age (years)", y = "Connectivitiy Index",
...
p_sex_facet <- ggplot(data_age_restricted, 
                      aes(x = eeg_age, y = left, color = affected_group)) +
  geom_point(alpha = 0.4, size = 2) +
  geom_smooth(method = "gam", formula = y ~ s(x, k=5), 
  ...
  labs(
    title = "Connectivitiy Trajectories by Group and Sex",
    x = "Age (years)", 
    y = "Connectivitiy Index",
  ) +
  theme_minimal(base_size = 14) +
  theme(
...
m3_simplified <- gam(left ~ affected_group + 
...
ggplot(diff_data, aes(x = eeg_age, y = ASD_vs_NonAff, 
  ...
  labs(
    ...
    x = "Age (years)",
    y = "Difference in Connectivitiy Index\n(ASD - Non-Affected)",
    color = "Sex",
    linetype = "Sex"
  ) +
